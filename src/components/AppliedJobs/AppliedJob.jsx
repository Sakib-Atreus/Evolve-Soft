  ...
  return (
    <div className="my-container">
      <div className="flex justify-end my-5 mr-3">
        <select
          className="bg-orange-100 p-2 rounded-md outline-none border-none"
        ...
        {isFilter.map((job) => (
          <div
            ...
            <div className="lg:space-y-2 space-y-1">
              ...
              <button className="bg-indigo-400 font-bold text-white p-2 rounded-md">{job?.work_type}</button>{" "}
              <button className="bg-indigo-400 font-bold text-white p-2 rounded-md">{job?.worktime}</button>
            ...
            <div className="ml-auto">
              <Link to={`/details/${job?.id}`}>
                <button className="bg-red-500 font-bold text-white p-2 rounded-md">View Details</button>
              </Link>
            </div>
          </div>
        ))}
