...
const DetailsJob = ({ job }) => {
  ...
  return (
    <div className="border p-5 space-y-2 relative">
      ...
      <span className="bg-indigo-400 font-bold text-white p-2 rounded-md">{work_type}</span>{" "}
      <span className="bg-indigo-400 font-bold text-white p-2 rounded-md">{worktime}</span>
      ...
      <Link to={`/details/${id}`}>
        <button
          onClick={() => navigate(id)}
          className=" bottom-2 bg-gradient-to-t  from-red-500 to-indigo-500 roun
